...
<STYLE TYPE="text/css">
	<!--
		.node { color: black;
			...
			font-size : 9pt;}
		.node A:link { color: #0000FF; text-decoration: underline; }
		.node A:visited { color: #0000FF; text-decoration: underline; }
		.node A:active { color: #0000FF; text-decoration: underline; }
		.node A:hover { color: #FF00FF; text-decoration: underline; }
	-->
	</STYLE>
<title>Knights of Columbus - Bishop Thomas E. Gill Assembly #1955</title>
  ...
  <tr>
    <td><div align="center"><img src="Images/4thdegree.gif" width="56" height="75"></div></td>
  </tr>
</table>
					...
					if (sImageSource.indexOf("empty") == -1)
					{
						if (targetElement.style.display == "none")
						{
							...
							if (srcElement.className == "LEVEL1")
								...
								srcElement.src = "images/menu/minusonly.gif";
							else
								//Otherwise, just show the open folder icon
								srcElement.src = "images/menu/folderopen.gif";
						}
						else
						{
							...
							if (srcElement.className == "LEVEL1")
								...
								srcElement.src = "images/menu/plusonly.gif";
							else
								//Otherwise, just show the closed folder icon
								srcElement.src = "images/menu/folderclosed.gif";
						}
					} 
				}
			}
...
Sub DisplayNode(ByVal objNodes, ByRef iElement, ByRef sLeftIndent, byVal arOpenFolders)
	...
	Dim oNode, sAttrValue, sNodeType, sURL,sTarget
	Dim NODE_ELEMENT
...
' alert
	For Each oNode In objNodes
		...
		if oNode.nodeType = NODE_ELEMENT Then
			...
			sTarget = oNode.getAttribute("target")
				...
				case "document" %>
					<table border="0" cellspacing="0" cellpadding="0" width="100%">
						<tr valign="bottom">
							...
							%>
							<td width="31"><img src="images/menu/<%=fnChooseIcon(bIsLast, sNodeType, bHasChildren, bForceOpen)%>" width="31" height="16" border="0"></td>
							<td title='<%=sAttrValue%>' nowrap class="node">&nbsp;<a href="<%=sURL%>" target="<%=sTarget%>"><%=sAttrValue%></a></td>
						</tr>
					</table>
				...
				case else %>
					<table border="0" cellspacing="0" cellpadding="0" width="100%">
						<tr valign="bottom">
							...
							%>
							<td width="31"><img class="LEVEL<%=iElement%>" src="images/menu/<%=fnChooseIcon(bIsLast, sNodeType, bHasChildren, bForceOpen)%>" id="<%=iElement%>" width="31" height="16" border="0"></td>
							<td title='<%=sAttrValue%>' nowrap class="node">&nbsp;<% if len(trim(sURL)) > 0 then %><a href="<%=sURL%>" target="basefrm"><%=sAttrValue%></a><% else %><%=sAttrValue%><% end if %></td>
						</tr>
					</table>
...
function fnChooseIcon(byval bIsLast, byval sNodeType, byval bHasChildren, byval bForceOpen)
	...
	if (sNodeType = "root") then 
		sIcon = "house.gif"
	elseif (sNodeType = "document") then  'We always pass an empty value here when setting a document image
...
function fnBuildLeftIndent(byval oNode, byval bIsLast, byVal sLeftIndent)
	...
	if (bIsLast = false) then
		...
		sLeftIndent = sLeftIndent & "<td width=""18""><img src=""images/menu/line.gif"" width=""18"" height=""16""></td>"
	else
		'Otherwise it is the last on the list so we just need to
		'display a blank space
		sLeftIndent = sLeftIndent & "<td width='20'><img src=""images/menu/pixel.gif"" width='20' height=""1"" border=""0""></td>"
	end if
